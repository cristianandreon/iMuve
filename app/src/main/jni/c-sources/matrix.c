 ...
 * \ingroup matrix
 */
void _lib3ds_matrix_add(Lib3dsMatrix m, Lib3dsMatrix a, Lib3dsMatrix b)
{
	int i,j;
 ...
 * \ingroup matrix
 */
void _lib3ds_matrix_sub(Lib3dsMatrix m, Lib3dsMatrix a, Lib3dsMatrix b)
{
	int i,j;
...
static Lib3dsFloat det3x3( Lib3dsFloat a1, Lib3dsFloat a2, Lib3dsFloat a3, Lib3dsFloat b1, Lib3dsFloat b2, Lib3dsFloat b3, Lib3dsFloat c1, Lib3dsFloat c2, Lib3dsFloat c3)
{
...
void
lib3ds_matrix_trasnform ( Lib3dsMatrix M, float *x, float *y, float *z ) {
	Lib3dsVector pos, out;

	pos[0] = *x;
	pos[1] = *y;
	pos[2] = *z;

	lib3ds_vector_transform(out, M, pos);
...
void
lib3ds_matrix_create ( 	Lib3dsMatrix M
						,float tx, float ty, float tz
						,float sx, float sy, float sz
						,float rx, float ry, float rz
						) {

	Lib3dsMatrix MRx, MRy, MRz, MS, MT;

	lib3ds_matrix_identity(MRx);
	lib3ds_matrix_identity(MRy);
	lib3ds_matrix_identity(MRz);
	lib3ds_matrix_identity(MS);
	lib3ds_matrix_identity(MT);

	lib3ds_matrix_scale_xyz(MS, sx, sy, sz);

	lib3ds_matrix_rotate_x(MRx, rx);
	lib3ds_matrix_rotate_y(MRy, ry);
	lib3ds_matrix_rotate_z(MRz, rz);

	lib3ds_matrix_translate_xyz(MT, tx, ty, tz);


	lib3ds_matrix_mult(M, MT);
	lib3ds_matrix_mult(M, MRx);
	lib3ds_matrix_mult(M, MRy);
	lib3ds_matrix_mult(M, MRz);
	lib3ds_matrix_mult(M, MS);
}

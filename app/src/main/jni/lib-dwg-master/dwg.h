...
typedef struct _dwg_entity_HATCH
{
  ...
  Dwg_Entity_HATCH_Path* paths;
	...
	// AGGIUNTA del 7/8/2015
	BITCODE_BS __unknown_style;
	BITCODE_BS __patterntype;
	BITCODE_BS __numdeflines; 

	BITCODE_BD __angle;
	BITCODE_BD __scaleorspacing;
	BITCODE_B __doublehatch;

	BITCODE_2DPOINT __offset;
	BITCODE_2DPOINT __pt0;
	BITCODE_BS __numdashes;
	BITCODE_BD __dashlength;

	BITCODE_BL __numseedpoints;
	BITCODE_2RD __seed_pt;
	BITCODE_BD __pixelsize;
...
#define DWG_UNKNOWN1_SIZE 123
  ...
  long unsigned int num_objects;
    long unsigned int num_objects_allocated;
  Dwg_Object *object;
...
double dwg_page_y_max(Dwg_Data *dwg);
      ...
      extern int dwg_read_data(char *data, unsigned int data_size, Dwg_Data *dwg_data);
      // extern void draw_dwg ( void *ptr_dwg );

...
// 9-8-2015 
// Presente un errore di lettura da risolvere  
return;
      ...
      if (FIELD_VALUE(version)==1)
        {
          do
            {
              FIELD_VALUE(sat_data) = (BITCODE_RC**) my_realloc(FIELD_VALUE(sat_data), (i+1) * sizeof(BITCODE_RC*));
              FIELD_VALUE(block_size) = (BITCODE_BL*) my_realloc(FIELD_VALUE(block_size), (i+1) * sizeof(BITCODE_BL));
  ...
  REPEAT(num_paths, paths, Dwg_Entity_HATCH_Path)
    {
      ...
      if (FIELD_VALUE(paths[rcount].flag) & 4) {
		_obj->__anypixelsize = 1;
		}
...
{	unsigned int rcount = 0, rcount2 = 0;
	...
	if (!FIELD_VALUE(solid_fill)) {
		...
		FIELD_BD (__scaleorspacing);

		// doublehatch			B		77  1 for double hatch
		FIELD_B (__doublehatch);

		// numdeflines			BS		78  number of definition lines
		FIELD_BS (__numdeflines);
		

		// Repeat numdeflines times:
		for (rcount=0; rcount < FIELD_VALUE(__numdeflines); rcount++) {
			...
			FIELD_BS (__numdashes);
			
			// Repeat numdashes times:
			for (rcount2=0; rcount2 < FIELD_VALUE(__numdashes); rcount2++) {
				// dashlength      BD  49  dash length
				FIELD_BD (__dashlength);
				}
			}
		}

	if (FIELD_VALUE(__anypixelsize)) {
		// pixelsize        BD  47  pixel size
		FIELD_BD(__pixelsize);
		}

	// numseedpoints      BL  98  number of seed points
	{	
		FIELD_BL(__numseedpoints);
	
		// Repeat numseedpoints times:
		for (rcount=0; rcount < FIELD_VALUE(__numseedpoints); rcount++) {

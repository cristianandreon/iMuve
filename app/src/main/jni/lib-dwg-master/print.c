...
#define FIELD_VECTOR_N(name, type, size)\
	if (size>0) {\
		...
		for (_vcount=0; _vcount< size; _vcount++) {\
			LOG_TRACE(#name "[%d]: " FORMAT_##type "\n", _vcount, _obj->name[_vcount])\
			}\
		}
...
#define FIELD_2RD_VECTOR(name, size)\
	{	unsigned int __vcount;\
		for (__vcount=0; __vcount< _obj->size; __vcount++) {\
			FIELD_2RD(name[__vcount]);\
			}\
		}
...
#define FIELD_2DD_VECTOR(name, size)\
	...
	{	unsigned int ___vcount;\
		for (___vcount = 1; ___vcount < _obj->size; ___vcount++) {\
			FIELD_2DD(name[___vcount], FIELD_VALUE(name[___vcount - 1].x), FIELD_VALUE(name[___vcount - 1].y));\
			}\
		}

#define FIELD_3DPOINT_VECTOR(name, size)\
	{	unsigned int ____vcount;\
		for (____vcount=0; ____vcount< _obj->size; ____vcount++) {\
			FIELD_3DPOINT(name[____vcount]);\
			}\
		}

#define HANDLE_VECTOR_N(name, size, code)\
	{	unsigned int __hvcount;\
		for (__hvcount=0; __hvcount<size; __hvcount++) {\
			FIELD_HANDLE(name[__hvcount], code);\
			}\
		}
...
#define REACTORS(code)\
	{	unsigned int __vcount;\
		for (__vcount=0; __vcount<obj->tio.object->num_reactors; __vcount++) {\
			FIELD_HANDLE(reactors[__vcount], code);\
			}\
		}
...
#ifdef TEST_ANDROID_JNI

	void __android_log_print(int type, char *msg, char *txt) {
	}

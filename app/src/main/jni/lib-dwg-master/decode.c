...
extern unsigned int GLAllocatedBytes;
...
#define FIELD_HANDLE(name, handle_code)\
  ...
  /* LOG_TRACE(#name ": HANDLE(%d.%d.%lu) absolute:%lu\n",_obj->name->handleref.code,_obj->name->handleref.size,_obj->name->handleref.value,_obj->name->absolute_ref)*/
...
#define FIELD_VECTOR_N(name, type, size)\
	if (size>0) {\
		unsigned int __vcount; \
		_obj->name = (BITCODE_##type*) calloc(1,size * sizeof(BITCODE_##type));\
		for (__vcount=0; __vcount< size; __vcount++) {\
			_obj->name[__vcount] = bit_read_##type(dat);\
...
#define FIELD_2RD_VECTOR(name, size)\
	_obj->name = (BITCODE_2RD *) calloc(1,_obj->size * sizeof(BITCODE_2RD));\
	{	unsigned int _vcount;\
		for (_vcount=0; _vcount< _obj->size; _vcount++) {\
			FIELD_2RD(name[_vcount]);\
			}\
		}
...
#define FIELD_2DD_VECTOR(name, size)\
	...
	FIELD_2RD(name[0]);\
	{	unsigned int __vcount;\
		for (__vcount = 1; __vcount < _obj->size; __vcount++) {\
			FIELD_2DD(name[__vcount], FIELD_VALUE(name[__vcount - 1].x), FIELD_VALUE(name[__vcount - 1].y));\
			}\
		}

#define FIELD_3DPOINT_VECTOR(name, size)\
	_obj->name = (BITCODE_3DPOINT *) calloc(1,_obj->size * sizeof(BITCODE_3DPOINT));\
	{	unsigned int ___vcount;\
		for (___vcount=0; ___vcount< _obj->size; ___vcount++) {\
			FIELD_3DPOINT(name[___vcount]);\
			}\
		}

#define HANDLE_VECTOR_N(name, size, code)\
  FIELD_VALUE(name) = (BITCODE_H*) calloc(1,sizeof(BITCODE_H) * size);\
	{	unsigned int ____vcount;\
		for (____vcount=0; ____vcount<size; ____vcount++) {\
			FIELD_HANDLE(name[____vcount], code);\
			}\
		}
...
#define REACTORS(code)\
  FIELD_VALUE(reactors) = (BITCODE_H*) calloc(1,sizeof(BITCODE_H) * obj->tio.object->num_reactors);\
	{	unsigned int fv__vcount;\
		for (fv__vcount=0; fv__vcount<obj->tio.object->num_reactors; fv__vcount++) {\
			FIELD_HANDLE(reactors[fv__vcount], code);\
			}\
		}

#define ENT_REACTORS(code)\
  FIELD_VALUE(reactors) = (BITCODE_H*) calloc(1,sizeof(BITCODE_H) * obj->tio.entity->num_reactors);\
	{	unsigned int _fv__vcount;\
		for (_fv__vcount=0; _fv__vcount<obj->tio.entity->num_reactors; _fv__vcount++) {\
			FIELD_HANDLE(reactors[_fv__vcount], code);\
			}\
		}
...
#define REPEAT_N(times, name, type) \
	if (times) _obj->name = (type *) calloc(1,times * sizeof(type));\
	for (rcount=0; rcount<times; rcount++)

#define REPEAT(times, name, type) \
	if (_obj->times) _obj->name = (type *) calloc(1,_obj->times * sizeof(type));\
...
static void \
 dwg_decode_##token (Bit_Chain * dat, Dwg_Object * obj) {\
  ...
  obj->tio.entity = (Dwg_Object_Entity*)my_calloc(1,sizeof (Dwg_Object_Entity));	\
  obj->tio.entity->tio.token = (Dwg_Entity_##token *)my_calloc (1,sizeof (Dwg_Entity_##token)); \
...
#define DWG_OBJECT(token) static void  dwg_decode_ ## token (Bit_Chain * dat, Dwg_Object * obj) {\
  ...
  obj->tio.object = (Dwg_Object_Object*)my_calloc(1,sizeof (Dwg_Object_Object));	\
  obj->tio.object->tio.token = (Dwg_Object_##token * ) my_calloc (1,sizeof (Dwg_Object_##token)); \
...
dwg_decode_data(Bit_Chain * dat, Dwg_Data * dwg)
{
  ...
  if (dwg->header.version == 0)
    {
        sprintf(GLLogStr, "Invalid or unimplemented version code - This file's version code is: %s\n", version);
        LOG_ERROR(GLLogStr)
        return -1;
    }
  ...
  //This line should not be reached!
    sprintf(GLLogStr, "ERROR: LibDWG does not support this version: %s.\n", version);
    LOG_ERROR(GLLogStr)
  return -1;
}
...
unsigned int idc = 0, idc_allocated = 0;
...
// Still unknown values: 6 'zeroes' and a 'one'
  ...
  for (i = 0; i < 7; i++) {
      sig = bit_read_RC(dat);
      LOG_TRACE("0x%02X [pos:%d]", sig, dat->byte);
    }
  LOG_TRACE("\n")
   ...
   */
  dwg->header.section = (Dwg_Section*) my_calloc(1,sizeof(Dwg_Section) * dwg->header.num_sections);
  ...
  if (dwg->header.num_sections == 6)
    {
      ...
      dwg->unknown1.chain = (unsigned char*)my_calloc(1,dwg->unknown1.size);
      ...
      if (bit_search_sentinel(dat, dwg_sentinel(DWG_SENTINEL_PICTURE_END)))
        {
          ...
          dwg->picture.chain = (unsigned char *)my_calloc(1,dwg->picture.size+1);
  ...
  LOG_INFO("         Header Variables (end): %8X\n", (unsigned int) (dwg->header.section[0].address + dwg->header.section[0].size))
  ...
  ckr2 = bit_ckr8(0xc0c1, dat->chain + dwg->header.section[0].address + 16, dwg->header.section[0].size - 34);
  ...
  LOG_INFO("         CLASS (end): %8X\n", (unsigned int) (dwg->header.section[1].address + dwg->header.section[1].size))
  ...
  i = 0;
    ...
    do {
        ...
        if (idc >= idc_allocated) {
            if (idc == 0)
                idc_allocated = 1;
                else
                idc_allocated = idc + 256;
            dwg->dwg_class = (Dwg_Class *) my_realloc(dwg->dwg_class, (idc_allocated) * sizeof(Dwg_Class));
            }
      ...
      dwg->dwg_class[idc].item_class_id = bit_read_BS(dat);
        ...
        #ifdef DEBUG_LEVEL1
            LOG_INFO("%d - class name: %s - %s\n", idc, dwg->dwg_class[idc].dxfname?dwg->dwg_class[idc].dxfname:"", dwg->dwg_class[idc].appname?dwg->dwg_class[idc].appname:"");
        #endif
  ...
  do {
      ...
      if (section_size > 2048) {
          LOG_ERROR("Object-map section size greater than 2048!\n")
          return -1;
        }
      ...
      while (dat->byte - duabyte < section_size) {
          ...
          if (dat->byte == previous_address) break;
          //if (dat->byte - duabyte >= seksize)
          //break;

          if (object_end < last_address) object_end = last_address;
          if (object_begin > last_address) object_begin = last_address;
      ...
      if (ckr != ckr2) {
            LOG_TRACE("section %d crc todo ckr:%x ckr2:%x\n", dwg->header.section[2].number, ckr, ckr2);
            return -1;
            }

    if (dat->byte >= maplasta) break;

    // LOG_TRACE("end section size : %d\n", section_size);
...
resolve_objectref_vector(Dwg_Data * dwg) {
  ...
  for (i = 0; i < dwg->num_object_refs; i++) {
      // LOG_TRACE("\n==========\n") LOG_TRACE("-objref: HANDLE(%d.%d.%lu) Absolute:%lu\n", dwg->object_ref[i]->handleref.code, dwg->object_ref[i]->handleref.size, dwg->object_ref[i]->handleref.value, dwg->object_ref[i]->absolute_ref)
      ...
      if (DWG_LOGLEVEL >= DWG_LOGLEVEL_INSANE)
        {
          if (obj)
            ...
            sprintf(GLLogStr, "Null object pointer: object_ref[%lu]\n", i);
            LOG_ERROR(GLLogStr)
          }
        }
    }
...
{
  char *decomp = NULL, *ptr = NULL;
  int i = 0, i_allocated = 0;
  int section_address;
  int bytes_remaining;
  ...
  decomp = (char *)my_calloc(1, decomp_data_size+1);
  if (decomp == 0)
    return;   // No memory
  ...
  section_address = 0x100;  // starting address
	...
	while(bytes_remaining) {
            ...
            // LOG_TRACE("Start alloc : %d", dwg->header.num_sections)
			/*
			if (dwg->header.num_sections==0)
				dwg->header.section = (Dwg_Section*) my_calloc(1,sizeof(Dwg_Section));
			else
				dwg->header.section = (Dwg_Section*) my_realloc(dwg->header.section, sizeof(Dwg_Section) * (dwg->header.num_sections+1));
            */
            ...
            if (i >= i_allocated) {
                ...
                dwg->header.section = (Dwg_Section*) my_realloc(dwg->header.section, sizeof(Dwg_Section) * (i_allocated));
                }

            // LOG_TRACE("Done alloc : %d", dwg->header.section)
		  ...
		  // LOG_TRACE("Section Number/Size/Addr:[%d][%x][%x] - [remain:%d]\n",   dwg->header.section[i].number, dwg->header.section[i].size, dwg->header.section[i].address, bytes_remaining)


          /// LOG_TRACE("Section addr:%d - size:%d - num_sections:%d", dwg->header.section, sizeof(Dwg_Section) * (dwg->header.num_sections+1), dwg->header.num_sections)
                        ...
                        unsigned long int decomp_data_size)
{
  ...
  dwg->header.section_info = (Dwg_Section_Info*)my_calloc(1,sizeof(Dwg_Section_Info) * dwg->header.num_descriptions);
  ...
  for (i = 0; i < dwg->header.num_descriptions; ++i)
    {
      ...
      dwg->header.section_info[i].sections = (Dwg_Section**)my_calloc(1,dwg->header.section_info[i].num_sections * sizeof(Dwg_Section*)+1);
	...
	{
        sprintf(GLLogStr, "section count %ld in area %d too high! skipping\n", dwg->header.section_info[i].num_sections, i );
        LOG_ERROR(GLLogStr)
	}
    }
  FREE_POINTER(decomp);
                            ...
                            long unsigned int section_type)
{
  ...
  decomp = (char *)my_calloc(1,max_decomp_size+1);
  if (decomp == 0)
    return 2;   // No memory
...
read_2004_section_classes(Bit_Chain* dat, Dwg_Data *dwg)
{
  ...
  if (bit_search_sentinel(&sec_dat, dwg_sentinel(DWG_SENTINEL_CLASS_BEGIN))) {
      ...
      do {
            ...
            /*
            if (idc == 0)
            dwg->dwg_class = (Dwg_Class *) my_calloc(1,sizeof(Dwg_Class));
            else
            dwg->dwg_class = (Dwg_Class *) my_realloc(dwg->dwg_class, (idc + 1) * sizeof(Dwg_Class));
            */
...
decode_R2004(Bit_Chain* dat, Dwg_Data * dwg)
{
  ...
  for (i = 0; i < 0x54; i++)
    {
      sig = bit_read_RC(dat);
      if (sig != 0 && loglevel) {
        LOG_ERROR("Warning: Byte should be zero! But a value=%x was read instead.\n")
        }
    }
  ...
  if (dwg->header.section_info != 0) {
	...
	{ FREE_POINTER(dwg->header.section_info[i].sections); }
...
decode_R2007(Bit_Chain* dat, Dwg_Data * dwg)
{
  ...
  resolve_objectref_vector(dwg);
    ...
    sprintf(GLLogStr, "Decoding of DWG version R2007 header is not fully implemented yet. we are going to try\n");
    LOG_ERROR(GLLogStr)
  return 0;
}
...
dwg_decode_entity(Bit_Chain * dat, Dwg_Object_Entity * ent)
{
  ...
  if (error)
    {
        sprintf(GLLogStr, "dwg_decode_entity:\tError in object handle! Current Bit_Chain address: 0x%0x\n");
        LOG_ERROR(GLLogStr)
  ...
  while (size = bit_read_BS(dat)) {
      ...
      if (size > 10210) {
            sprintf(GLLogStr, "dwg_decode_entity: Absurd! Extended object data size: %lu. Object: %lu (handle).\n", (long unsigned int) size, ent->object->handle.value);
            LOG_ERROR(GLLogStr)
          ent->bitsize = 0;
          ent->extended_size = 0;
          ent->picture_exists = 0;
      ...
      if (ent->extended_size == 0)
        {
          ent->extended = (char *)my_calloc(1,size+1);
          ent->extended_size = size;
        }
      else
        {
          ent->extended_size += size;
          ent->extended = (char *)my_realloc(ent->extended, ent->extended_size+1);
        }
  ...
  if (ent->picture_exists)
    {
      ...
      if (ent->picture_size < 210210)
        {
          ent->picture = (char *)my_calloc(1,ent->picture_size+1);
          for (i = 0; i < ent->picture_size; i++)
            ent->picture[i] = bit_read_RC(dat);
        }
      else
        {
            sprintf(GLLogStr, "dwg_decode_entity:  Absurd! Picture-size: %lu kB. Object: %lu (handle).\n", ent->picture_size / 1000, ent->object->handle.value);
            LOG_ERROR(GLLogStr)
...
dwg_decode_object(Bit_Chain * dat, Dwg_Object_Object * ord)
{
  ...
  if (error)
    {
        sprintf(GLLogStr, "\tError in object handle! Bit_Chain current address: 0x%0x\n", (unsigned int) dat->byte);
        LOG_ERROR(GLLogStr)
  ...
  while (size = bit_read_BS(dat))
    {
      if (size > 10210)
        {
            sprintf(GLLogStr, "dwg_decode_object: Absurd! Extended object data size: %lu. Object: %lu (handle).\n", (long unsigned int) size, ord->object->handle.value);
            LOG_ERROR(GLLogStr)
          ord->bitsize = 0;
          ord->extended_size = 0;
          ord->num_handles = 0;
      ...
      if (ord->extended_size == 0)
        {
          ord->extended = (unsigned char *)my_calloc(1,size+1);
          ord->extended_size = size;
        }
      else
        {
          ord->extended_size += size;
          ord->extended = (unsigned char *)my_realloc(ord->extended, ord->extended_size+1);
        }
...
static Dwg_Object_Ref *dwg_decode_handleref(Bit_Chain * dat, Dwg_Object * obj, Dwg_Data* dwg) {
  // Welcome to the house of evil code!
  Dwg_Object_Ref* ref = (Dwg_Object_Ref *) my_calloc(1,sizeof(Dwg_Object_Ref));

  if (bit_read_H(dat, &ref->handleref)) {
      if (obj) {
            sprintf(GLLogStr, "Could not read handleref in object whose handle is: %d.%d.%lu\n", obj->handle.code, obj->handle.size, obj->handle.value);
            LOG_ERROR(GLLogStr)
        } else {
  ...
  if (dwg->num_object_refs == 0)
    dwg->object_ref = (Dwg_Object_Ref **) my_calloc(1,REFS_PER_REALLOC * sizeof(Dwg_Object_Ref*));
  else
    if (dwg->num_object_refs % REFS_PER_REALLOC == 0) {
        dwg->object_ref = (Dwg_Object_Ref **) my_realloc(dwg->object_ref, (dwg->num_object_refs + REFS_PER_REALLOC) * sizeof(Dwg_Object_Ref*));
      }
      */
      // Maybe not so fair
    if (dwg->num_object_refs % REFS_PER_REALLOC == 0) {
        // LOG_TRACE("dwg->object_ref reallocating %d", dwg->object_ref);
        dwg->object_ref = (Dwg_Object_Ref **) my_realloc(dwg->object_ref, (dwg->num_object_refs + REFS_PER_REALLOC) * sizeof(Dwg_Object_Ref*));
        // LOG_TRACE("dwg->object_ref reallocated at %d to %d [%d items]", dwg->num_object_refs, dwg->object_ref, dwg->num_object_refs + REFS_PER_REALLOC);
        }
...
static Dwg_Object_Ref *dwg_decode_handleref_with_code(Bit_Chain * dat, Dwg_Object * obj, Dwg_Data* dwg, unsigned int code) {
  Dwg_Object_Ref * ref;
  ...
  if (ref->absolute_ref == 0 && ref->handleref.code != code) {
        sprintf(GLLogStr, "Expected a CODE %d handle, got a %d\n", code, ref->handleref.code);
        LOG_ERROR(GLLogStr)
...
dwg_decode_xdata(Bit_Chain * dat, int size)
{
  ...
  while (dat->byte < end_address)
    {
      rbuf = (Dwg_Resbuf *) my_calloc(1,sizeof(Dwg_Resbuf));
      rbuf->next = 0;
      ...
      switch (get_base_value_type(rbuf->type))
        ...
        case VT_STRING:
          ...
          if (length > 0)
            {
              rbuf->value.str = (char *)my_calloc(1,length+1);
              for (i = 0; i < length; i++)
        ...
        case VT_BINARY:
          ...
          if (rbuf->value.chunk.size > 0)
            {
              rbuf->value.chunk.data = (char *)my_calloc(1,rbuf->value.chunk.size+1);
        ...
        case VT_OBJECTID:
          ...
          break;
        default:
            sprintf(GLLogStr, "Invalid group code in xdata: %d!\n", rbuf->type);
            LOG_ERROR(GLLogStr)
          FREE_POINTER(rbuf);
...
dwg_decode_variable_type(Dwg_Data * dwg, Bit_Chain * dat, Dwg_Object* obj)
{
  ...
  i = obj->type - 500;
    ...
    // LOG_TRACE("dwg_decode_variable_type: i:%d - type:%s", i, (const char *)dwg->dwg_class[i].dxfname);
    ...
    long unsigned int address)
{
   ...
   */
   ...
   if (dwg->num_objects >= dwg->num_objects_allocated) {
        if (dwg->num_objects == 0)
            dwg->num_objects_allocated = 1;
            else
            dwg->num_objects_allocated = dwg->num_objects+256;
        #ifdef DEBUG_LEVEL1
            LOG_TRACE("dwg_decode_add_object:extending to %d objs allocated", dwg->num_objects_allocated);
            #endif
        dwg->object = (Dwg_Object *) my_realloc(dwg->object, (dwg->num_objects_allocated + 1) * sizeof(Dwg_Object));
    }


if (!dwg->object) {
        LOG_TRACE("dwg_decode_add_object:out of memory");
  ...
  */
  ...
  default:
	...
	if(obj->tio.unknown) {
		FREE_POINTER(obj->tio.unknown);
	}

        /*
        obj->tio.unknown = (unsigned char*)my_calloc(1,obj->size);
        if(obj->tio.unknown) {
            memcpy(obj->tio.unknown, &dat->chain[object_address], obj->size);
        } else {
            LOG_TRACE("dwg_decode_add_object:failed to allocate %d bytes", obj->size);
        }
        */
  ...
  /* Register the previous addresses for return */
    ...
    #if DEBUG_LEVEL2
        LOG_INFO("Done1 [dat addr:%d]", (int)&dat)
        #endif
